    ...
    for action in actions:
        ...
        for sequence in range(int(no_sequences)):
            ...
            for frame_num in range(sequence_length):
                ...
                image_rh_v, results_rh_v = md.mediapipe_detection(cv2.flip(frame, 1), holistic) #amostras invertidas na horizontal

                # Inserindo pontos na imagem
                dl.draw_landmarks(image_rh_v, results_rh_v)

                # Legendas que indicam qual amostra e gesto está sendo coletado
                # UTIL PARA SABER QUANDO COMEÇA E QUANDO TERMINA CADA COLETA
                if frame_num == 0:
                    cv2.rectangle(image_rh_v, (0, 0), (640, 40), (245, 117, 16), -1)
                    cv2.putText(image_rh_v, 'INICIANDO COLETA', (120, 200),
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1, cv2.LINE_AA)
                    cv2.putText(image_rh_v, f'Demonstrando "{action}" -- amostra numero {sequence} -- {frame_num}', (15, 20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
                    cv2.waitKey(
