...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    ...
    for action in actions:
        ...
        no_sequences_rh_v = int(no_sequences) #sequencias das amostras invertidas na horizontal
        no_sequences_h_rv = no_sequences_rh_v + int(no_sequences) #sequencias das amostras invertidas na vertical
        no_sequences_rh_rv = no_sequences_h_rv + int(no_sequences) #sequencias das amostras invertidas tanto na vertical como na horizontal

        for sequence in range(int(no_sequences)):
            print(sequence)
            print(no_sequences_rh_v)
            print(no_sequences_h_rv)
            print(no_sequences_rh_rv)
            print("")
            ...
            for frame_num in range(sequence_length):
                ...
                image_rh_v, results_rh_v = md.mediapipe_detection(cv2.flip(frame, 0), holistic) #amostras invertidas na horizontal
                image_h_rv, results_h_rv = md.mediapipe_detection(cv2.flip(frame,1), holistic) #amostras invertidas na vertical
                image_rh_rv, results_rh_rv = md.mediapipe_detection(cv2.flip(frame,-1), holistic) #amostras invertidas tanto na vertical como na horizontal
                ...
                keypoints_h_rv = ek.extract_keypoints(results_h_rv) #amostras invertidas na vertical
                keypoints_rh_rv = ek.extract_keypoints(results_rh_rv) #amostras invertidas tanto na vertical como na horizontal

                # Pasta onde o frame será salvo
                npy_path_h_v = os.path.join(DATA_PATH, action, str(sequence),str(frame_num)) #amostras originais
                npy_path_rh_v = os.path.join(DATA_PATH, action, str(no_sequences_rh_v), str(frame_num)) #amostras invertidas na horizontal
                npy_path_h_rv = os.path.join(DATA_PATH, action, str(no_sequences_h_rv), str(frame_num)) #amostras invertidas na vertical
                npy_path_rh_rv = os.path.join(DATA_PATH, action, str(no_sequences_rh_rv), str(frame_num)) #amostras invertidas tanto na vertical como na horizontal
                ...
                #np.save(npy_path_h_rv, keypoints_h_rv) #amostras invertidas na vertical
                #np.save(npy_path_rh_rv, keypoints_rh_rv) #amostras invertidas tanto na vertical como na horizontal
            ...
            no_sequences_h_rv = no_sequences_h_rv + 1  # amostras invertidas na vertical
            no_sequences_rh_rv = no_sequences_rh_rv + 1  # amostras invertidas tanto na vertical como na horizontal
